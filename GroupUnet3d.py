...
class Down(nn.Module):
    def __init__(self, in_channels, out_channels, lifting=False):
        ...
        grid_H = R.quat_to_matrix(R.cyclic_z(4))
        
        if self.lifting:
            self.C1 = GLiftingConvSE3(in_channels, out_channels, kernel_size=3, padding='same', permute_output_grid=False, grid_H=grid_H)
        else:
            self.C1 = GSeparableConvSE3(in_channels, out_channels, kernel_size=3, padding='same', permute_output_grid=False,  grid_H=grid_H)
        
        self.C2 = GSeparableConvSE3(out_channels, out_channels, kernel_size=3, padding='same', permute_output_grid=False,  grid_H=grid_H)
